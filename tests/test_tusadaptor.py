...
class TestTUSAdaptor:
    ...
    def test_get_mkt_equ_d_by_codelist(self,tus:TUSAdaptor ):
        trade_date = date(2022,1,28)
        db = DBAdaptor()
        df_expect= db.getDfBySql("select id, sec_id, list_date from stock.equity where list_status_cd='L' and exchange_cd='XSHG' order by id desc limit 2")
        ...
        df = tus.getMktEquDByCodeList(df_expect['sec_id'], start_date=start_date, end_date = end_date)
        assert df is not None and df.shape[0]>0, "返回的df不能为空"
