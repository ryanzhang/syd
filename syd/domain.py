...
class Equity(Base):
    ...
    id = Column(Integer, Sequence('equity_id_seq', schema='stock'), primary_key=True)
    ...
    non_rest_float_a = Column(Float) # t    无限售流通股本(最新)。如果为A股，该列为最新无限售流通A股股本数量；如果为B股，该列为最新流通B股股本数量
...
class TradeCalendar(Base):
    ...
    id = Column(Integer, Sequence('trade_calendar_id_seq', schema='stock'), primary_key=True)
...
class MktIdxDay(Base):
    ...
    id = Column(Integer, Sequence('mkt_idx_day_id_seq', schema='stock'), primary_key=True)
...
class MktEquDay(Base):
    ...
    id = Column(Integer, Sequence('mkt_equ_day_id_seq', schema='stock'), primary_key=True)
    ...
    accum_adj_bf_factor = Column(Float) # accumAdjBfFactor;  //累积前复权因子，前复权价=未复权价*累积前复权因子。前复权是对历史行情进行调整，除权除息当日的行情无需调整。最近一次除权除息日至最新交易日期间的价格也无需调整，该期间前复权因子等于1。
...
class FundDay(Base):
    ...
    id = Column(Integer, Sequence('fund_day_id_seq', schema='stock'), primary_key=True)
    sec_id = Column(String(255)) # secID; //    通联编制的证券编码，可使用DataAPI.SecIDGet获取
...
class Fund(Base):
    ...
    id = Column(Integer, Sequence('fund_id_seq', schema='stock'), primary_key=True)
    sec_id = Column(String(255)) # secID; //    通联编制的证券编码，可使用DataAPI.SecIDGet获取
    ticker = Column(String(255)) #ticker; //    通用交易代码
    sec_short_name = Column(String(255)) # secShortName; //    证券简称
...
class SyncStatus(Base):
    ...
    id = Column(Integer, Sequence('sync_status_id_seq', schema='stock'), primary_key=True)
    rc = Column(Boolean) 
