...
class TUSAdaptor:
    ...
    def getStockBasicInfo(self) -> tuple[pd.DataFrame, str]:
        ...
        if self.is_use_cache and os.path.exists(df_cache_file):
            ...
            try:
                logger.debug(
                    "Loading Data from tushare stock_basic interface remotely::"
                )
                df = self.conn.stock_basic(
                ...
                logger.error(
                    "loading data from tus failure" + traceback.format_exc()
                )
    ...
    def getMktEquD(self, trade_date: datetime, ts_code=""):
        ...
        for count in range(3):
            try:
                logger.debug(
                    "Loading Data from tushare daily interface remotely::"
                )
                df = self.conn.daily(
    ...
    def getMktEquDExtra(self, trade_date: datetime, ts_code=""):
        ...
        for count in range(3):
            try:
                logger.debug(
                    "Loading Data from tushare daily_basic interface remotely::"
                )
    ...
    def getMktEquDhfq(self, trade_date: datetime, ts_code=""):
        ...
        for count in range(3):
            try:
                logger.debug(
                    "Loading Data from tushare adj_factor interface remotely::"
                )
    ...
    def getMktEquDByCodeList(
        ...
        for index, sec_id in sec_ids.items():
            ts_code_str_list = (
                ...
                + sec_id[0:6]
                + "."
                + TUSAdaptor.db_code_mapper[sec_id[7:11]]
                + ","
            )
        ...
        logger.info(
            f"Start to fetch {ts_code_str_list} during {start_date} {end_date} from tus "
        )
        df = pd.DataFrame()
        # 允许重试3次
        for count in range(3):
    ...
    def getFundDaily(self, trade_date: datetime, ts_code=""):
        ...
        for count in range(3):
            try:
                logger.debug(
                    "Loading Data from tushare fund_daily interface remotely::"
                )
    ...
    def getFundDayhfq(self, trade_date: datetime, ts_code=""):
        ...
        for count in range(3):
            try:
                logger.debug(
                    "Loading Data from tushare fund_adj interface remotely::"
                )
