...
class TestStockSync:
    ...
    def test_equity_has_been_sync_to_latest(self, syncer:StockSyncer, db:DBAdaptor):
        ...
        df_db_sync = db.getDfBySql("select * from stock.sync_status where table_name= 'equity'")
        assert df_db_sync is not None
        assert df_db_sync.iloc[0]['update_time'].date() == date.today(), "sync_status已经被更新"

    def test_trade_calendar_has_been_sync_to_latest(self, syncer:StockSyncer, db:DBAdaptor):
        df_incremental = syncer.sync_trade_calendar()
        assert df_incremental is not None, "update 不能返回None"

        df_db =  db.getDfBySql("select calendar_date from stock.trade_calendar order by trade_calendar desc limit 1")
    ...
    def test_mkt_equ_day_basic_has_been_fetch_from_tus(self, syncer:StockSyncer):
    ...
    def test_mkt_equ_day_extra_has_been_fetch_from_tus(self, syncer:StockSyncer):
        df = syncer.sync_mkt_equity_day()
    ...
    #复权
    def test_mkt_equ_day_hfq_has_been_fetch_from_tus(self, syncer:StockSyncer):
        df = syncer.sync_mkt_equity_day()
    ...
    def test_mkt_equ_day_has_been_update_to_latest(self, syncer:StockSyncer, db:DBAdaptor):
        df1 = pd.read_pickle("/tmp/mkt_equ_day_15_28.pkl")
        df2 = pd.read_pickle("/tmp/mkt_equ_day_extra_15_28.pkl")
        df2 = df2[['ts_code', 'trade_date', 'pe', 'pb']]
        df3 = pd.read_pickle("/tmp/mkt_equ_day_hfq_15_28.pkl")
        ...
        df_result = db.getDfBySql("select * from stock.mkt_equ_day \
            where ticker='000001' order by trade_date desc limit 30")
        assert df_result is not None
        assert df_result.shape[0]>1
        assert df_result.iloc[0]['trade_date'] == syncer.getLatestTradeDate(), \
            "数据库中没有找到最后一个交易日数据"
        ...
        df_sync_status = db.getDfBySql("select * from stock.sync_status \
        ...
        assert df_sync_status.iloc[0]['update_time'].date() == datetime.today().date(), "字段不能空"


    @skip
    def test_mkt_equity_day_integrity(self,  db:DBAdaptor, syncer:StockSyncer):
        pass
